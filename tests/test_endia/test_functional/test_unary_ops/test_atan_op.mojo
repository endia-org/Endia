...
def run_test_atan(msg: String = "atan"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40))
    arr_torch = nd.utils.to_torch(arr)

    res = nd.atan(arr)
    res_torch = torch.atan(arr_torch)
...
def run_test_atan_grad(msg: String = "atan_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.atan(arr))
    res_torch = torch.sum(torch.atan(arr_torch))
...
def run_test_atan_complex(msg: String = "atan_complex"):
    torch = Python.import_module("torch")
    arg0 = nd.randn_complex(List(2, 30, 40))
    arg0_torch = nd.utils.to_torch(arg0)

    res = nd.atan(arg0)
    res_torch = torch.atan(arg0_torch)
