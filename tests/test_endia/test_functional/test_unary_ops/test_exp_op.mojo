...
def run_test_exp_grad(msg: String = "exp_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.exp(arr))
    res_torch = torch.sum(torch.exp(arr_torch))
...
def run_test_exp_complex(msg: String = "exp_complex"):
    torch = Python.import_module("torch")
    arg0 = nd.randn_complex(List(2, 30, 40))
    arg0_torch = nd.utils.to_torch(arg0)

    res = nd.exp(arg0)
    res_torch = torch.exp(arg0_torch)
