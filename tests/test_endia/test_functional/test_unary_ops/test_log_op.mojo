...
def run_test_log_grad(msg: String = "log_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.log(arr))
    res_torch = torch.sum(torch.log(arr_torch))
...
def run_test_log_complex(msg: String = "log_complex"):
    torch = Python.import_module("torch")
    arg0 = nd.randn_complex(List(2, 30, 40))
    arg0_torch = nd.utils.to_torch(arg0)

    res = nd.log(arg0)
    res_torch = torch.log(arg0_torch)
