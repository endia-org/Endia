...
def run_test_sigmoid(msg: String = "sigmoid"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40))
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sigmoid(arr)
    res_torch = torch.sigmoid(arr_torch)
...
def run_test_sigmoid_grad(msg: String = "sigmoid_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.sigmoid(arr))
    res_torch = torch.sum(torch.sigmoid(arr_torch))
