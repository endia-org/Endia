...
def run_test_sin_grad(msg: String = "sin_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.sin(arr))
    res_torch = torch.sum(torch.sin(arr_torch))
...
def run_test_sin_complex(msg: String = "sign_complex"):
    torch = Python.import_module("torch")
    arg0 = nd.randn_complex(List(2, 30, 40))
    arg0_torch = nd.utils.to_torch(arg0)

    res = nd.sin(arg0)
    res_torch = torch.sin(arg0_torch)
