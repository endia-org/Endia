...
def run_test_square(msg: String = "square"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40))
    arr_torch = nd.utils.to_torch(arr)

    res = nd.square(arr)
    res_torch = torch.square(arr_torch)
...
def run_test_square_grad(msg: String = "square_grad"):
    torch = Python.import_module("torch")
    arr = nd.randn(List(2, 30, 40), requires_grad=True)
    arr_torch = nd.utils.to_torch(arr)

    res = nd.sum(nd.square(arr))
    res_torch = torch.sum(torch.square(arr_torch))
...
def run_test_square_complex(msg: String = "square_complex"):
    torch = Python.import_module("torch")
    arg0 = nd.randn_complex(List(2, 30, 40))
    arg0_torch = nd.utils.to_torch(arg0)

    res = nd.square(arg0)
    res_torch = torch.square(arg0_torch)
